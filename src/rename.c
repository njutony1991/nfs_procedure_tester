...
void nfs_rename_testcase_to_longname_cb(struct rpc_context *rpc, int status, void *data, void *private_data) {
    ...
    construct_long_name(RENAME_PATHCONF.name_max);
...
void nfs_rename_testcase_prepare_cb(struct rpc_context *rpc, int status, void *data, void *private_data) {
    ...
    if (rpc_nfs3_pathconf_async(rpc, nfs_rename_testcase_from_longname_cb, &args, client) != 0) {
...
int main(int argc, char *argv[]) {
    ...
    client.test_case_cb = nfs_rename_testcase_prepare_cb; 
